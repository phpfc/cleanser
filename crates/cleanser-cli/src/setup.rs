...
use cleanser_core::{DirectoryCategory, FileSystemCrawler, Platform, WhitelistConfig};
...
fn get_setup_marker_path() -> anyhow::Result<PathBuf> {
...
fn mark_setup_complete() -> anyhow::Result<()> {
...
/// Crawler progress for setup
struct SetupCrawlerProgress {
    pb: ProgressBar,
}

impl SetupCrawlerProgress {
    fn new() -> Self {
...
impl cleanser_core::CrawlerProgress for SetupCrawlerProgress {
...
pub fn run_first_time_setup() -> anyhow::Result<()> {
    ...
    if build_map {
        ...
        let progress = Box::new(SetupCrawlerProgress::new());
...
fn get_whitelist_suggestions(home: &Path, platform: Platform) -> Vec<(PathBuf, &'static str)> {
