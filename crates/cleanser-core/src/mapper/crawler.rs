//! Filesystem crawler for building the directory map.
//!
//! This module provides the crawler that walks the filesystem and classifies
//! directories. It uses callbacks for progress updates instead of directly
...
/// Callback trait for crawler progress updates
pub trait CrawlerProgress: Send + Sync {
...
pub struct NoOpCrawlerProgress;

impl CrawlerProgress for NoOpCrawlerProgress {
    fn on_start(&self, _message: &str) {}
    fn on_progress(&self, _current: usize, _total: usize, _path: &Path) {}
...
pub struct FileSystemCrawler {
    ...
    progress: Option<Box<dyn CrawlerProgress>>,
}
...
impl FileSystemCrawler {
    ...
    pub fn with_progress_callback(mut self, callback: Box<dyn CrawlerProgress>) -> Self {
        self.progress = Some(callback);
        self
    }

    /// Legacy method for compatibility - creates a no-op progress
    pub fn with_progress(self, _show_progress: bool) -> Self {
        // In the core library, we don't have indicatif
        // The CLI will use with_progress_callback instead
        self
    }
    ...
    pub fn crawl_full(&self) -> Result<FileSystemMap> {
        ...
        if let Some(ref progress) = self.progress {
            progress.on_start("Starting filesystem scan...");
        }
        ...
        for (idx, dir) in scan_dirs.iter().enumerate() {
            ...
            if let Some(ref progress) = self.progress {
                progress.on_progress(idx + 1, scan_dirs.len(), dir);
            }
        ...
        if let Some(ref progress) = self.progress {
            progress.on_complete("Filesystem scan complete");
        }
    ...
    pub fn quick_scan(&self) -> Result<FileSystemMap> {
        ...
        if let Some(ref progress) = self.progress {
            progress.on_start("Quick scanning common locations...");
        }
        ...
        if let Some(ref progress) = self.progress {
            progress.on_complete("Quick scan complete");
        }
    ...
    pub fn smart_scan(&self, existing_map: &mut FileSystemMap) -> Result<()> {
        if let Some(ref progress) = self.progress {
            progress.on_start("Smart scanning filesystem...");
        }
        ...
        if let Some(ref progress) = self.progress {
            progress.on_complete("Smart scan complete");
        }
