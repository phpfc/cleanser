...
/// Enhanced scan using the filesystem mapper
pub fn scan_with_mapper(config: ScanConfig) -> Result<ScanResults> {
    println!("{}", "Starting intelligent filesystem scan...".cyan());
...
fn deduplicate_nested_paths(mut items: Vec<CleanableItem>) -> Vec<CleanableItem> {
    items.sort_by(|a, b| {
        let len_cmp = b.path.as_os_str().len().cmp(&a.path.as_os_str().len());
